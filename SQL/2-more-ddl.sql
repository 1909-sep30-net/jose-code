...
Go
CREATE FUNCTION ReturnInitial(@ID int)
RETURNS nvarchar
AS
BEGIN
    ...
    select @FirstName = substring(FirstName,1,1) from Customer where CustomerId = @ID;
	select @LastName = substring(LastName,1,1) from Customer where CustomerId = @ID;
